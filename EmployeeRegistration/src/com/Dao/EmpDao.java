...
public class EmpDao {
	public static void insertEmployee(Employee e) {
		try {
			...
			String sql= "insert into emp_registration(fname,lname,email,mobile,gender,address)values(?,?,?,?,?,?)";
	...
	public static void deleteEmployee(int id) {
		try {
			Connection conn=EmpUtil.CreateConnection();
			String sql="delete from emp_registration where id=?";
			PreparedStatement pst=conn.prepareStatement(sql);
			pst.setInt(1, id);
			pst.executeUpdate();
		}catch(Exception e1) {
	...
	public static void updateEmployee(Employee e) {
		try {
			Connection conn=EmpUtil.CreateConnection();
			String sql="update emp_registration set fname=?,lname=?,email=?,mobile=?,gender=?,address=? where id=?";
			PreparedStatement pst= conn.prepareStatement(sql);
			pst.setString(1, e.getFname());
			pst.setString(2, e.getLname());
			pst.setString(3, e.getEmail());
